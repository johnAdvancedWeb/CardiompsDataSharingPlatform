  ...
  <div id="section-one">
    <div class="min-h-screen p-10">
      <div class="space-y-10 md:space-y-0">
        ...
        <!-- right -->
        <div v-for="(ed, index) in experimentalData" :key="index" style="padding: .5%">

          <div>
            <apexchart type="line" height="350" width="500" :options="chartOptions" :series="series"></apexchart>
          </div>
          <button @click="updateChart(ed.y1Axis, ed.y2Axis)">Generate Graph for this post</button>
          <div class="latest-news-container">
            <div class="container">
              <div class="row">
                <div class="col">
                  <div id="post">
                    <div id="heading"><h1>{{ ed.title }}</h1></div>
                    <div id="content">
                      <p>{{ ed.description }}</p>
                      <p>{{ ed.content }}</p>
                      <p>{{ ed.xAxis }}</p>
                      <p>{{ ed.y1Axis }}</p>
                      <p v-if="ed.y2Axis.length">{{ ed.y2Axis }}</p>
                      <p v-if="ed.y3Axis.length">{{ ed.y3Axis }}</p>
                    </div>
                  </div>
                  <button @click="deleteData(index)" style="margin-top: 10px;">Delete Post</button>
                </div>
              </div>
            </div>
          </div>
